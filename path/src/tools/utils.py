...
#Updated to match user's data structure:
# - Images: data/fullFrame-210x260px/{train,dev,test}
# - Labels: data/label/{train,dev,test}.corpus.csv
def path_data(data_path=None, task='SLR', features_type='features', hand_query=None):

    #Path for the full frame
    train_data = os.path.join(data_path, "fullFrame-210x260px", "train")
    valid_data = os.path.join(data_path, "fullFrame-210x260px", "dev")
    test_data = os.path.join(data_path, "fullFrame-210x260px", "test")

    #Path for hands cropped images
    if(hand_query):
        hand_path = os.path.join(data_path, 'trackedRightHand-92x132px')
        train_hand = os.path.join(hand_path, "train")
        valid_hand = os.path.join(hand_path, "dev")
        test_hand = os.path.join(hand_path, "test")
    else:
        train_hand = None
        valid_hand = None
        test_hand = None

    # Annotations in label folder
    annotations = 'label'
