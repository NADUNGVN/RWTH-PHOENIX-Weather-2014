...
from tools.backbone import SignLanguageBackbone, BackboneFactory
...
class src_2Dembeddings(nn.Module):
    """
    Image embedding module supporting multiple backbone architectures.
    
    Supported backbones:
        - 'mb2': MobileNetV2 (original, lightweight)
        - 'efficientnet_b0', 'efficientnet_b1', 'efficientnet_b2', 'efficientnet_b3': EfficientNet variants
        - 'resnet18', 'resnet34', 'resnet50', 'resnet101': ResNet variants
        - 'convnext_tiny', 'convnext_small': ConvNeXt variants
    """
    ...
    def __init__(self, n_units, pretrained=True, image_size=224, network_type='mb2', channels=3):
        ...
        # Use new backbone module for modern architectures
        if network_type in BackboneFactory.SUPPORTED_BACKBONES:
            # Use new SignLanguageBackbone
            self.use_new_backbone = True
            self.backbone = SignLanguageBackbone(
                backbone_name=network_type,
                pretrained=pretrained,
                output_dim=n_units,
                dropout=0.2
            )
            self.network = self.backbone.backbone
            self.pool = self.backbone.pool
            self.projection = self.backbone.projection
            backbone_dim = BackboneFactory.FEATURE_DIMS[network_type]
        elif network_type == 'mb2':
            # Original MobileNetV2 implementation
            self.use_new_backbone = False
            ...
            if backbone_dim != n_units:
                self.projection = nn.Sequential(
                    nn.Dropout(0.2),
                    nn.Linear(backbone_dim, n_units),
            ...
            raise ValueError(f"Unsupported network type: {network_type}. "
                           f"Supported: {list(self.BACKBONE_DIMS.keys()) + BackboneFactory.SUPPORTED_BACKBONES}")
    ...
    def forward(self, x):
        ...
        # Reshape to join seq size with batch: (B*T, C, H, W)
        ...
        if len(feature_map.shape) > 2:
            ...
            frame_embeddings = pooled.flatten(1)
        else:
            frame_embeddings = feature_map
        
        # Project to target dimension
        frame_embeddings = self.projection(frame_embeddings)
